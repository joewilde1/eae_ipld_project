...
unique_country_count = temps_df['Country'].nunique()
...
if unique_cities_list is not None and len(selected_cities) > 0:
    ...
    plt.title(f'Average temperature in {", ".join(selected_cities[:-1])} and {selected_cities[-1]} between {start_date} and {end_date}')
    plt.xlabel('Date')
    ...
    plt.title(f'Distribution of average temperatures in {city} between {start_date} and {end_date}')  
